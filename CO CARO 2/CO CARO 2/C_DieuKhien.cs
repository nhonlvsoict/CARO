...
namespace CO_CARO_2
{
    class C_DieuKhien
    {
        ...
        public void undo(Graphics g)
        {
            if (_sanSang)
            {
                if (_stkCacNuocDaDi.Count > 2 && _cheDoChoi == 2)
                {
                    C_OCo oCo;
                    for (int i = 1; i <= 2; i++)
                    {
                        oCo = _stkCacNuocDaDi.Pop();
                        BanCo.xoaQuanCo(g, oCo.Cot * C_OCo.CHIEU_CAO, oCo.Dong * C_OCo.CHIEU_RONG);
                        MangOCo[oCo.Dong, oCo.Cot].SoHuu = 0;
                    }
                }
                else if (_stkCacNuocDaDi.Count > 0 && _cheDoChoi == 1)
                {
                    C_OCo oCo;
                    oCo = _stkCacNuocDaDi.Pop();
                    BanCo.xoaQuanCo(g, oCo.Cot * C_OCo.CHIEU_CAO, oCo.Dong * C_OCo.CHIEU_RONG);
        ...
        private void gayBan(int soHuu, String message = "")
        {
            if (soHuu == 1)
            {
            ...
            else if (soHuu == 2)
            {
                ...
                String gay = arrGay[rd.Next(0, 3)];
                PictureChuoi1.Image = global::CO_CARO_2.Properties.Resources.gif4edit;
                TextGayBan.Text = gay;

                PanelGayBan.Visible = true;
                //Delayed(4000, () => PanelGayBan.Visible = false);
            }
            else if (_stkCacNuocDaDi.Count > 5 && _stkCacNuocDaDi.Count % 4 == 0 && _cheDoChoi == 2)
            {
                String[] arrGay = { "You gonna lose 😏", "You almost got me 😎", "Your move is predicted 😱", "You think I can't see that 😛", "Interested move 😘", "Nice try! 😁" };
                String gay = arrGay[rd.Next(0, 6)];
                PictureChuoi1.Image = global::CO_CARO_2.Properties.Resources.gif31;
                TextGayBan.Text = gay;
